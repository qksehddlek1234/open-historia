...
|---|---|---|
| **Leader line** | seat only, `ownScale < LEADER_AREA_SCALE_FLOOR = 20000` (San Marino scores 589; Danzig 9,116) | `buildLeaderPlacement` puts the label outside the shape, horizontal, at `LEADER_LABEL_AREA_SCALE = 30000`, on a hairline from the territory's edge (`leaderLines` collection); `labelLineExtension` (a map setting, default `LEADER_EXTENSION_DEFAULT = 0.5`) is how far out. The way out is the minor axis, northern (else eastern) side — a convention, so a cluster of micro-states leans one way. Once every label is placed (step 8), a leader label that lands on another label — 80 pairs sat on each other fleet-wide, 36 under a country's name, and the leader layer is collision-culled — walks `leaderPlacementCandidates` (the other side, the major axis both ways, the diagonals, then the same eight further out) and takes the first clear spot; the line follows (fleet: 80 → 4 pairs, 36 → 3, 96 of 548 moved) |
| **Curved** | seat only, no leader, and the flat name would not fit: `widestLineEm(name) > NAME_FIT_EM × √elongation` — or, after placement (step 8), a seat whose flat label sits on its land nowhere (`landCoverage < LABEL_CURVE_LAND_COVERAGE = 0.8`: Honshu, Vietnam's S, the Solomons — 22 seats fleet-wide) | `buildOwnerCurve` traces a centreline through the cluster's own rings along the same axis, and `layoutGlyphsAlongPath` emits **one Point feature per glyph** (`curved: 1`), sized to fill the path within `[LEADER_AREA_SCALE_FLOOR, ownScale]`; the shape-driven kind is laid compact and centred on the path (`maxPerEm`) since its name is short; no curve if the path will not hold the name legibly or bends too hard between glyphs (Honshu's "일본 제국" stays flat) — GRAND-HESSE and SAXE-WEIMAR on the original's 1836 board are this rung |
