...
| # | 항목 | 결과 |
|---|---|---|
| P2 | `Internal:` 자문 10종 + `Domestic:` 신문·민간인 | ✅ `267aed3` |
| P3 | 점령 폴리티 명명 · 예정 이벤트 카드 · 비역사 반응 | ✅ `76a24f8` |
| 고급 | 통합 주기 정박 수리 | ✅ `a2622f6` |
| — | 아이슬란드: eraSovereignty 덴마크 확정 + 근거 명기 | ✅ `a2622f6` |
| 고급 | 지도 렌더링 + 규칙 무게 계기판 | ✅ `af99913` (일곱 중 여섯) |
| P4-② | 번왕국 | ✅ `23087c5` — 60개가 아니라 **다섯** |
| 고급 | 라벨 선 확장 | ❌ **스케일할 대상이 없다** — 우리 국가 라벨은 도형 위에 얹히거나 도형을 따라 휘고, 유도선을 아예 안 그린다. 설정이 아니라 플랜 G 렌더 작업 |
| — | `gameMaster` 스텁(1,006자) 채우기 | ⏭ **다음 배치** — 12슬롯 중 유일한 실질 공백 |
| P4-① | 중국 지급시 세분화(33 → ~300) | ⏭ **미룬다** — GADM 중국 level-2 시드 선행, 한 나라만 촘촘해지면 영국 218개 때와 같은 문제 |
| — | 폴리티 태그(tags) 개념 | ⏭ **미룬다** — DMZ/점령 판별에 쓰이나 우리는 명명 규칙으로 대체했다 |

### 번왕국을 다섯으로 줄인 이유

원본은 60여 개를 개별 폴리티로 두지만 우리 지역은 현대 level-1이라 그 해상도가
없고, 대부분이 1지역짜리가 되면 우리 지역 계약상 "1지역 = 나라 전체"라 서사
부담만 는다. **현대 지역 하나가 통째로 번왕국 땅이었던 다섯**만 꺼냈다 —
잠무카슈미르(3조각 = 도그라 왕조 하나) · 라지푸타나 · 하이데라바드 · 마이소르 ·
트라방코르코친. 뒤 둘은 근사다(마이소르 ≈ 카르나타카 남부, 트라방코르 ≈ 케랄라
남부; 북부는 각각 봄베이·마드라스 관구였다).

공직자 스위트가 다섯을 전부 빈손으로 잡아냈고 — 빈손이 곧 12B의 창작이라는
R1의 교훈 — 넷은 실존 군주를, 라지푸타나는 단일 군주가 없는 청이라 이 저장소의
doctrine대로 사람 대신 제도를 적었다.
