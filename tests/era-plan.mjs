...
test("A DATE IT WILL NOT RUN IS NAMED, with the reason", () => {
  ...
  // will actually occur: ww1-1914 is a real board, --probe will give it a
  // polity dump, and nobody has chosen its window yet. Taken away again
  // whatever happens — the first draft of this used a date no board wants and
  // learned that the planner, correctly, does not plan for those at all.
  const date = "1914-07-28";
  ...
  try {
    ...
    assert.match(out, /skipped — noWindow: 1914-07-28/, "the date has to be named, not just counted");
