...
export default {
  ...
  eraGeometry: {
    ...
    faceKeepOut: {
      ...
      // ── 같은 병 둘 더 (전 보드 검사에서 나왔다) ─────────────────────────
      // 검사 규칙: **작은 나라 이름 + 큰 주 링**. 14보드 면 전부를 훑어 19건이
      // 걸렸고 17건은 정상이었다(토스카나 2.25deg²≈22,000km² = 실제, 메클렌부르크
      // 1.78≈13,000 = 실제, 파르마 0.69≈5,600 = 실제). 남은 둘이 진짜다.
      //
      // 뤼베크 자유시: 주 링 2.086deg²(≈15,000km²)에 bbox 8.6~11.1°E × 53.4~54.9°N —
      // **슐레스비히홀슈타인 전체**다. 실제 뤼베크는 약 300km². 작은 링 15개(합
      // ≈350km²)가 진짜 영토이고 주 링만 가짜다. 실측: 그 면이 `DEU.DEF0`을
      // 덴마크에게서 빼앗고 있었다 — 스펙이 `"DEU.15_1": "DAN"`으로 준 그 칸이다.
      //
      // 헤센홈부르크: 주 링 0.545deg²(≈4,200km²), 실제 약 275km². 이건 모양이 다르다 —
      // 홈부르크(8.6°E)와 마이젠하임 월경지(7.7°E) **두 조각을 하나로 이어** 그
      // 사이 프랑크푸르트·비스바덴·다름슈타트를 통째로 삼켰다.
      //
      // ★ 대가를 적어 둔다: 울타리는 칸 단위라 **두 나라가 지도에서 사라진다**
      // (뤼베크 300km²·홈부르크 275km²는 NUTS 칸보다 작다). 대신 슐레스비히홀슈타인
      // 15,800km²가 덴마크로, 다름슈타트 권역 7,400km²가 제 주인에게 돌아온다.
      // **로스터에는 남긴다** — 실존 국가이고, 조립기가 가짜 링을 떼는 순간 면이
      // 지역을 이기므로 저절로 다시 그려진다. 그게 진짜 수리이고 조립 레인의 일이다.
      "Freie und Hansestadt Lübeck": ["DEU.DEF0"],
      "Hessen-Homburg": ["DEU.DE71"],
  ...
  regionAssignments: {
    ...
    // ★ B-6 (2026-08-20): 위 문장("헤자즈·티하마만")은 처음부터 옳았는데 **지도가
    // 그 말을 못 지켰다.** GADM level-1의 SAU는 13블록 평균 148,257 km²로 시드 전역
    // 평균의 5.4배짜리 이상치였고, 그중 Makkah 주 하나가 홍해안에서 동쪽 43.83°E
    // (메카에서 423 km)까지 이어지는 단일 덩어리다. 그래서 제다·메카를 주는 순간
    // 나즈드 내륙까지 함께 칠해졌다 — 실측 이집트 SAU 446,037 km² 중 191,250 km²
    // (42.9%)가 40°E 동쪽. 사우디 전체의 23.1%다.
    //
    // 시드를 GADM level-2로 확장해(147행, `scripts/regions/merge-level2.mjs`) 이제
    // 군(郡) 단위로 적을 수 있다. 아래는 **헤자즈 축**이다 — 홍해 항구(제다·얀부·
    // 라비그·움루즈·두바·알와즈), 성지와 그 배후(메카·메디나·타이프·바드르·카이바르),
    // 그리고 티하마 해안. 산맥을 넘어가는 아홉 군은 나즈드 몫으로 아래에 따로 적었다.
    //
    // 타부크(SAU.13)·알바하(SAU.2)·지잔(SAU.10)은 통째로 유지한다. 앞의 둘은 헤자즈
    // 북부와 아시르 산지의 이집트 주둔지이고, 지잔은 티하마 해안이다 — 40°E 동쪽
    // 지표가 이 남쪽 위도에서는 오히려 오도한다(지잔의 해안선 자체가 42°E다).
    "SAU.13_1": "EGY", "SAU.2_1": "EGY", "SAU.10_1": "EGY",
    // 메디나 축 — 얀부(항구) · 메디나 · 바드르 · 알이스 · 알울라 · 와디알파라 · 카이바르
    "SAU.5.9_1": "EGY", "SAU.5.3_1": "EGY", "SAU.5.6_1": "EGY", "SAU.5.2_1": "EGY",
    "SAU.5.5_1": "EGY", "SAU.5.8_1": "EGY", "SAU.5.7_1": "EGY",
    // 메카 축 — 제다(항구) · 메카 · 타이프 · 라비그 · 쿨라이스 · 알주뭄 · 바흐라 ·
    // 알카밀 · 알리스 · 알쿤푸다 · 아담 · 마이산
    "SAU.11.11_1": "EGY", "SAU.11.13_1": "EGY", "SAU.11.8_1": "EGY", "SAU.11.15_1": "EGY",
    "SAU.11.12_1": "EGY", "SAU.11.3_1": "EGY", "SAU.11.9_1": "EGY", "SAU.11.4_1": "EGY",
    "SAU.11.6_1": "EGY", "SAU.11.7_1": "EGY", "SAU.11.1_1": "EGY", "SAU.11.14_1": "EGY",
    "GRC.4.1_1": "EGY",
    "YEM.5_1": "EGY",
    ...
    // 산맥 동쪽 아홉 군 — 헤자즈가 아니라 나즈드 변경이다. 투루바와 알쿠르마는
    // 1919년 헤자즈-나즈드 전쟁이 벌어진 바로 그 경계 지대이고, 알히나키야·알마흐드는
    // 메디나 동쪽 사막이다. 이집트가 리야드를 치는 것은 1838년이므로 1836년의
    // 이 땅은 파이살의 것이다.
    "SAU.11.17_1": "NEJ", "SAU.11.5_1": "NEJ", "SAU.11.16_1": "NEJ", "SAU.11.2_1": "NEJ",
    "SAU.11.10_1": "NEJ", "SAU.5.1_1": "NEJ", "SAU.5.4_1": "NEJ",
    ...
    // ══ 보고 5 — 이집트 영토: **타우루스까지 간다** ═══════════════════════
    // 앞 배치에서 이미 고친 셋(반도에서 헤자즈만 남기기 · 남수단 제외 · 크레타
    // 추가)과 **다른 건이다.** 남은 것은 북쪽 끝이다.
    //
    // 이 파일 머리글이 스스로 적어 뒀다 — *"Muhammad Ali ruling from the Sudan to
    // the Taurus"*. 그런데 지도는 시리아 국경에서 멈춘다. **산문과 지도가 어긋나**
    // **있었다.**
    //
    // 1833-05 퀴타히아 협정이 무함마드 알리에게 시리아·다마스쿠스·알레포와 **아다나
    // 에얄레트**를 준다. 이브라힘 파샤가 그 경계를 타우루스 산맥으로 잡고, 더
    // 북쪽으로 밀고 올라가는 것은 **1839-06 니지프 전투** 때다 — 3년 뒤다.
    // 그래서 유프라테스 동쪽(샨르우르파·디야르바크르·말라티아)은 오스만에 남긴다.
    "TUR.1_1": "EGY", "TUR.58_1": "EGY", "TUR.64_1": "EGY", // 아다나 에얄레트 — 아다나·메르신(이첼)·오스마니예
    "TUR.37_1": "EGY", "TUR.33_1": "EGY", "TUR.48_1": "EGY", // 알레포 에얄레트의 아나톨리아 쪽 — 하타이·가지안테프·킬리스
    "TUR.42_1": "EGY", // 마라슈 — 이브라힘이 1832년에 얻었다
    ...
    // 다름슈타트 권역 — 홈부르크 울타리로 풀려난 칸이다. 슈타르켄부르크가 헤센
    // 대공국의 본령이고 그 수도가 다름슈타트다(기센 DE72도 같은 나라). 프랑크푸르트
    // 자유시와 나사우 몫은 각자 면이 있어 여기서 깎아 간다.
    "DEU.DE71": "HES",
