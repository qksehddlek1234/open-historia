...
export default {
  ...
  eraGeometry: {
    ...
    // 프로이센령 남프로이센이 맞다, 타슈켄트 밖). 라벨 착지 오류 2건(누에바
    // 에스파냐→이집트 잔여, 칠도공화국→펠로폰네소스)은 당일 중심 오버라이드
    // +재조립로 근본 수리됨: 누에바에스파냐 라벨은 창을 떠났고(면 자체가
    // 없어짐, 배정 29→28), 칠도공화국은 진짜 코르푸 섬 면(19.6,39.4→
    // 20.1,39.8 실측)에 앉았다. 배제 목록은 그래서 비어 있다.
    faceOwners: {
      ...
      // 칠도공화국: 러시아-오스만 공동보호 하의 자치 공화국(1800 콘스탄티노플
      // 조약) — 중심 수리 후 코르푸 면. 자기 이름으로 선다.
      "Ἑπτάνησος Πολιτεία": "Septinsular Republic",
