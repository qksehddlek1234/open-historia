...
export const REFERENCE = {
  ...
  "Mali Empire": {
    leader: [
      // 라구사(ㄱ-2 mongol-1300): 사쿠라 추정이나 사망 연대가 "1300년경"으로
      // 불확실 — 이븐 할둔도 현대 연구도 1300-01-01 재위자를 특정하지 못한다.
      { name: "만사 (사쿠라 추정 — 연대 불확실, 1300-01-01 재위자 특정 불가)", from: "1285-01-01", until: "1310-01-01" },
  ...
  // ── medieval-1200 확장 6: 아메리카 열한 자리 ────────────────────────────
  // 열하나 전부 기관이고, 이유가 아프리카와 또 다르다. 아프리카는 사람이
  // 다스렸는데 왕명부가 유실된 경우였다. 여기는 **기록의 성격 자체가 다르다** —
  // 카호키아와 마풍구브웨는 고고학만 남았고, 쿠스코의 초기 왕들은 잉카가
  // 문자를 쓰지 않아 구전이며, 치첸이차의 1200년 지배층은 연대기 판본마다
  // 다르다. 한 사람을 앉히는 순간 그 차이가 전부 지워진다.
  "Chichén Itzá": {
    // 1200년은 도시의 끝자락 — 마야판 연맹이 1220년경 뒤를 잇는다.
    leader: [{ name: "치첸이차의 지배층 (연대기 이설)" }],
  },
  "Mixtec–Zapotec City-States": {
    // 8-사슴 재규어발톱은 1063년 사람이다. 1200년의 오악사카는 여러 도시의 판이다.
    leader: [{ name: "미스텍·사포텍 도시들의 영주들 (단일 군주 없음)" }],
    headOfState: [{ name: "(없음 — 도시국가 무리)" }],
  },
  Chimor: {
    leader: [{ name: "찬찬의 왕 (탁아이나모 왕조 — 구전 계보)" }],
  },
  "Kingdom of Cusco": {
    // 잉카는 문자를 쓰지 않았고 초기 왕들은 정복 이후에 기록됐다.
    leader: [{ name: "쿠스코의 사파 잉카 (구전 계보)" }],
  },
  "Qulla Kingdom": {
    leader: [{ name: "콜라의 마이쿠 (아이마라 왕들)" }],
  },
  Chachapoya: {
    leader: [{ name: "차차포야 추장 연합 (단일 군주 없음)" }],
    headOfState: [{ name: "(없음 — 추장국 연합)" }],
  },
  "Muisca Confederation": {
    // 수장이 하나가 아니라 둘이다 — 그게 이 연맹의 구조다.
    leader: [{ name: "바카타의 지파와 훈사의 자케 (수장 둘)" }],
    headOfState: [{ name: "(없음 — 지파·자케 이원 체제)" }],
  },
  Tairona: {
    leader: [{ name: "타이로나 마을들의 추장 (시에라네바다)" }],
  },
  Cahokia: {
    // 1200년의 카호키아는 런던보다 크다. 그런데 남은 것은 흙무덤과 유물뿐이다.
    leader: [{ name: "카호키아 몽크스마운드의 수장 (고고학 기록만)" }],
  },
  "Ancestral Puebloans": {
    // 차코가 비고(1150년경) 메사베르데로 옮겨 간 뒤의 공동체들 — 왕이 없다.
    leader: [{ name: "메사베르데 공동체들 (왕을 두지 않는 사회)" }],
    headOfState: [{ name: "(없음 — 공동체 사회)" }],
  },
  "Taíno Chiefdoms": {
    leader: [{ name: "타이노 카시케들 (섬별 추장국)" }],
    headOfState: [{ name: "(없음 — 추장국 연합)" }],
  },
  // ── medieval-1200 확장 7: 스텝·발트·게일 아일랜드 여덟 자리 ───────────
  // 다섯이 사람이고 셋이 기관이다. 그리고 이 배치의 사람들은 **곧 사라지거나
  // 곧 세계를 뒤집는다** — 옹칸과 타양칸은 6년 안에 테무진에게 무너지고,
  // 그 테무진은 지금 옹칸의 봉신이다.
  "Kingdom of Connacht": {
    leader: [{ name: "국왕 카할 크로브데르그 우아 콘초바르", from: "1189-01-01", until: "1224-05-28" }],
  },
  "Cenél nEógain": {
    leader: [{ name: "국왕 아드 메흐 우아 닐", from: "1196-01-01", until: "1230-01-01" }],
  },
  "Livonian Tribes": {
    // 리가 창건 1201, 검의 형제기사단 1202 — 이 보드는 그 직전이고, 여기엔
    // 아직 정복자도 단일 군주도 없다.
    leader: [{ name: "리보니아·라트갈리아 장로 회의 (단일 군주 없음)" }],
    headOfState: [{ name: "(없음 — 부족 연합)" }],
  },
  "Estonian Maakonds": {
    // 마아콘드는 향(鄕)이고, 전시에만 회의로 뭉쳤다.
    leader: [{ name: "에스토니아 마아콘드 회의 (전시에만 연합)" }],
    headOfState: [{ name: "(없음 — 향촌 연합)" }],
  },
  "Lithuanian Tribes": {
    // 민다우가스의 통합은 1230년대다.
    leader: [{ name: "리투아니아 부족 공들 (통합 전)" }],
    headOfState: [{ name: "(없음 — 부족 공국들)" }],
  },
  "Kerait Khanate": {
    // 1200년 몽골 고원의 패자. 1203년 테무진에게 무너진다.
    leader: [{ name: "옹칸 토그릴", from: "1171-01-01", until: "1203-01-01" }],
  },
  "Naiman Khanate": {
    leader: [{ name: "타양칸", from: "1198-01-01", until: "1204-01-01" }],
  },
  "Mongol Tribe": {
    // 1200년의 테무진은 칸이 아니다 — 옹칸의 의제 아들이자 봉신이고, 헨티
    // 산지의 다섯 지역이 그가 가진 전부다. 1206년 쿠릴타이에서 달라진다.
    leader: [{ name: "테무진 (옹칸의 봉신 — 칭기즈 칸 즉위 전)", from: "1189-01-01", until: "1206-01-01" }],
  },
  "Nepal Mandala": {
